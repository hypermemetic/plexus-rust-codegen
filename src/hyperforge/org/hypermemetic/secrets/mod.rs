//! Module for hyperforge.org.hypermemetic.secrets namespace
...
pub async fn delete(client: &PlexusClient, key: String) -> Result<Pin<Box<dyn Stream<Item = Result<SecretEvent>> + Send>>> {
    let stream = client.call_stream("hyperforge.org.hypermemetic.secrets.delete", json!({ "key": key })).await?;
...
pub async fn list(client: &PlexusClient) -> Result<Pin<Box<dyn Stream<Item = Result<SecretEvent>> + Send>>> {
    let stream = client.call_stream("hyperforge.org.hypermemetic.secrets.list", serde_json::Value::Null).await?;
