//! Module for hyperforge.org.juggernautlabs namespace
...
pub async fn schema(client: &PlexusClient) -> Result<serde_json::Value> {
    client.call_single("hyperforge.org.juggernautlabs.schema", serde_json::Value::Null).await
}

/// Show organization info
pub async fn info(client: &PlexusClient) -> Result<Pin<Box<dyn Stream<Item = Result<OrgEvent>> + Send>>> {
    let stream = client.call_stream("hyperforge.org.juggernautlabs.info", serde_json::Value::Null).await?;
