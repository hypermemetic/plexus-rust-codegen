//! Module for solar.jupiter.callisto namespace
...
pub async fn info(client: &PlexusClient) -> Result<Pin<Box<dyn Stream<Item = Result<SolarEvent>> + Send>>> {
    let stream = client.call_stream("solar.jupiter.callisto.info", serde_json::Value::Null).await?;
